...
+ 인증서 기반 공개키와 ID 기반 공개키의 차이점
  + 조금 더 자세하게 인증서 기반 공개키에 대해 단점을 찾아보았으나, 제대로 된 설명은 확인하지 못함. 
...
+ 아래의 세 가지 가정이 모두 유지되면 이 스킴은 random oracle chosen message and ID attack에 안전함.
...
+ 방법
  ...
  + 5) 데이터에 접근하기 위해서 유저는 우선 블록체인에서 포인터에 접근, (???? 서명한건데 왜 접근에 포인터로 접근이 필요?이용하면 되지??)
...
+ 결론
  ...
  + (개인의견) **데이터에 서명을 넣고 데이터를 주면, 제 3자가 이용(클라우드 컴퓨터가 머신러닝에 이용). 그 결과에는 내 서명이 이미 들어 있음. 그래서 여기에 내 데이터가 처리되었음을 알 수 있음**
